...
B+ 树是现代数据库系统中最广泛使用的索引数据结构之一，起源于 1970 年代对磁盘高效索引的研究。与传统的二叉搜索树不同，B+ 树通过大幅增加每个节点的分支因子（通常为数百），将树的高度控制在 3–4 层以内，从而在数十亿条记录中实现仅需 3–4 次磁盘 I/O 就能定位目标数据。MySQL InnoDB、PostgreSQL、SQLite、MongoDB 等主流数据库均以 B+ 树或其变体作为核心索引引擎。

## B+ 树作为数据引擎的核心优势

| 优势 | 说明 |
|------|------|
| **高扇出、低树高** | 每个节点容纳数百条记录，树高稳定在 O(log_{m} N)，百万级数据仅需 3–4 层寻址 |
| **范围扫描友好** | 叶子节点通过双向链表串联，范围查询无需回溯父节点，顺序 I/O 性能优异 |
| **空间局部性强** | 相邻键存储在相同或相邻页面中，利用磁盘预读（read-ahead）大幅提升扫描吞吐 |
| **写放大可控** | 节点分裂仅在满时发生，写入以页为单位批量化，避免随机小写入 |
| **稳定的查找性能** | 所有叶子在同一深度，任何键的访问路径长度一致，无退化风险 |
| **存储层解耦** | 页式管理天然对接文件系统块层，易集成缓冲池、WAL、checksum 等持久化机制 |

本项目从零实现一个完整的 B+ 树存储引擎，覆盖数据结构定义、页面管理、索引算法、事务处理、崩溃恢复等核心模块，旨在深入理解数据库内核的工作原理。

---

### 一、需求分析

这是一个**基于 B+ 树的数据存储引擎**，对外提供键值存储接口，并在此基础上构建了一个**世界财富排行榜**命令行应用。

> 核心功能包括：
> - **基础键值操作**：Put(key, value)、Get(key)、Delete(key)
> - **范围扫描**：通过游标（Cursor）支持按 key 顺序遍历
> - **持久化存储**：数据写入磁盘文件，支持关闭后重新打开
> - **缓冲池**：LRU 淘汰策略缓存热点页，减少磁盘 I/O
> - **WAL 预写日志**：先写日志再写数据，保障崩溃后数据可恢复
> - **事务**：基于读写锁的轻量级事务，支持多读一写并发
> - **自定义比较器**：键值视为不透明字节序列，用户可以传入自定义比较函数
