...
| # | 问题 | 所在模块 | 改进方向 |
|---|------|---------|---------|
| 1 | `txn_abort` 不撤销已做的修改 | `txn.c` | 结合 WAL 实现 Undo 日志，abort 时反向重放 |
| 2 | 删除后合并放不下时直接放弃，不尝试从兄弟借用 | `btree.c` | 实现键再分配（redistribution），使两页均衡 |
| 3 | LRU 淘汰遍历所有帧，O(capacity) | `buffer_pool.c` | 改用时钟算法或双向链表，降低到 O(1) |
| 4 | 已删除页号永不回收，文件只增不减 | `storage.c` | 维护空闲页号栈/位图，分配时优先复用 |
| 5 | 扩展文件时逐页写零，不支持稀疏文件 | `storage.c` | 改用 `ftruncate` / `fallocate`，避免零填充 I/O |
| 6 | WAL 恢复 `fclose` 后 `fopen` 可能失败导致数据丢失 | `wal.c` | 先写新头部并 `fflush`，确认成功后再交换文件 |
| 7 | Top-N / 排名操作为全表扫描 O(n) | `leaderboard.c` | 为财富值建二级索引，或维护固定大小堆结构 |
| 8 | 每次写操作直接写回页面 + WAL 追加，写放大明显 | 全局 | 引入写时复制（CoW）或 LSM-Tree 风格合并写 |
| 9 | 边界场景与并发测试覆盖不足 | `test/` | 增加模糊测试、TSan 并发安全测试、性能基准测试 |